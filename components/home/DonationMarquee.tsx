...
export function DonationMarquee() {
  ...
  const formatAmount = (n: number) => formatMoney(n, locale, tCommon)

  const timeAgo = (date: Date) => {
    const s = Math.floor((Date.now() - date.getTime()) / 1000)
    if (s < 60) return tCommon('relative.secondsAgo', { count: s })
    if (s < 3600) return tCommon('relative.minutesAgo', { count: Math.floor(s / 60) })
    return tCommon('relative.hoursAgo', { count: Math.floor(s / 3600) })
  }
        ...
        {items.map((ev, i) => (
          ...
          >
            ...
            <span className="text-sm text-foreground whitespace-nowrap">
              {t('line', {
                donor: `${ev.country} ${ev.donor}`,
                campaign: ev.campaign,
                amount: formatAmount(ev.amount),
              })}
            </span>
